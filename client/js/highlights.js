/* ═══════════════════════════════════════
   SilverVerse — Highlights (Auth + Gallery)
   ═══════════════════════════════════════ */
...
(function () {
  ...
  async function init() {
    var contentEl = document.getElementById('highlightsContent');
    ...
    try {
      ...
      contentEl.innerHTML =
        '<div class="highlights-auth-gate">' +
          '<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--gray-400)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="margin-bottom:16px"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>' +
          '<h2>Please login to view highlights</h2>' +
          '<p>You need an account to watch festival highlights.</p>' +
          '<a href="login.html?redirect=highlights.html" class="btn btn-primary">Login to Continue</a>' +
        '</div>';
      return;
    }

    var welcomeName = currentUser.displayName || currentUser.username || currentUser.email || '';
    contentEl.innerHTML =
      '<div class="highlights-welcome">' +
        '<p>Welcome, <strong>' + escapeHtml(welcomeName) + '</strong>! Browse and watch festival highlights below.</p>' +
      '</div>';

    document.getElementById('videosFilters').style.display = '';
    document.getElementById('videosGrid').style.display = '';
  ...
  function renderFilters() {
    var container = document.getElementById('videosFilters');
    if (!container) return;
